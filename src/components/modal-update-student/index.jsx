...
const ModalUpdateStudent = ({ open, handleClose, studentId }) => {
  const {
    setValue,
    register,
    handleSubmit,
  ...
  const { updateStudent, student, getStudent } = useStudent()
  ...
  const onSubmit = async (data) => {
    ...
    updateStudent(newData, student.id, accessToken).then(() => handleClose())
    toast.success('Estudante editado com sucesso!')
  }

  useEffect(() => {
    if (student) {
      const date = formatDate(student?.dataNascimento)
      setValue('nome', student.nome)
      setValue('idade', student.idade)
      setValue('dataNascimento', date)
      setValue('serie', student.serie)
      setValue('notaMedia', student.notaMedia)
      setValue('endereco', student.endereco)
